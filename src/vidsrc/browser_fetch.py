...
def fetch_past_challenge(url: str, referer: str | None, timeout_s: int) -> dict:
    ...
    try:
        ...
        # Plain driver.get() + passive waiting (what scraper.py's own
        # get_m3u8_urls_browser does for filmyfly's player hop) confirmed
        # NOT enough for this host: undetected-chromedriver alone clears
        # some Cloudflare setups just by not looking like a bot, but an
        # explicit Turnstile checkbox challenge still needs an actual
        # click. uc_open_with_reconnect() disconnects/reconnects chromedriver
        # around the navigation (standard UC-mode anti-detection technique),
        # then uc_gui_click_captcha() drives a real OS-level mouse click via
        # pyautogui - both no-op safely if there's nothing to click.
        driver.uc_open_with_reconnect(url, 4)
        time.sleep(1)
        try:
            driver.uc_gui_click_captcha()
        except Exception as e:
            print(f"[!] uc_gui_click_captcha failed (continuing to poll passively): {e}", file=sys.stderr)
