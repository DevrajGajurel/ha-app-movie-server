...
- Fixed a downloaded TV show rendering as a completely unrelated title - real report: House of the Dragon showing up in MediaNest's Library as "A Thousand and One Erotic Nights" (a 1982 film). TMDB numbers movies and TV shows in separate namespaces, so the same id means two different titles, and id 94997 happens to be both of these. `scanLibrary()` already knew this and decides a folder's type by whether it contains `S01`-style season subfolders; `listProgress()` instead guessed per progress *file*, treating one at the series-folder root as a movie. House of the Dragon has exactly that - a root-level progress file (from a whole-series play that resolved to the largest file rather than a specific episode) sitting alongside `S01/S02/S03` - so it reported one `type: "tv"` entry and one `type: "movie"` entry for the same show. `listProgress()` now uses the same season-subfolder rule as `scanLibrary()`, verified against the real library: both entries now come back as `tv`.
- MediaNest cached its by-id TMDB lookups (the fallback that recovers art for a title no longer on the scraped listing pages) in a map keyed by the bare numeric id, so the mistyped `movie` entry's result landed on the TV entry with the same number and replaced its title and poster wholesale. That cache, the library dedup key, and the catalog matchers (`matchMovieForDownload`/`matchMovieForProgress`) are now all keyed/compared by type *and* id, so a movie and a show sharing an id can no longer overwrite or dedup each other. The type is only compared when the catalog entry actually carries one, so untyped entries still match as before.

## 1.7.47

- Added a background sweep that proactively keeps each source domain's Cloudflare `cf_clearance` cookie from going cold on its own schedule, instead of only ever refreshing it as a side effect of a real user request. Checked every 10 minutes (cheap - just an in-memory lookup); only actually re-solves via FlareSolverr when a domain's cached clearance is missing or expiring within 15 minutes, so domains that hand out long-lived cookies (confirmed on filesdl.top: a full year) aren't re-solved for nothing. Re-fetches whichever real page URL last worked for that domain rather than guessing at a synthetic warm-up URL like a bare homepage, which could 404 or route differently and falsely look like "no challenge here."
- `cfClearanceCache.js` now persists to Redis (write-through, mirroring the pattern already used by the movie/probe/TMDB/download-options/job-history/Samsung-TV caches) instead of being in-memory only. A restart used to throw away every earned clearance even if it was still good for hours (or, per the filesdl.top case, most of a year) - forcing the next request to pay a full FlareSolverr solve again for no reason. Reads/writes elsewhere stay synchronous against the in-memory Map as before; Redis is purely a write-through backup, loaded back into memory on startup.
- Fixed downloaded TV series (e.g. "House of the Dragon") being invisible in the dashboard's "Downloaded Library" view despite being fully downloaded. Root cause: that view unconditionally filtered to `item.type === "movie"` - a leftover scope limit from when "Play on TV" (which needs a single file to launch) was first built, not an actual bug in the scan itself (`/api/downloads/library` already reported the item correctly, with `type: "tv"`). Downloaded series now show up in the grid like everything else, just without a "Play on TV" button (replaced with a short note) since there's still no per-episode picker to launch from here.
