...
- `flaresolverr_url` (optional) — a [FlareSolverr](https://github.com/alexbelgium/hassio-addons) instance you run yourself, used only as a fallback when a download-host redirect hits a genuine Cloudflare Turnstile challenge that plain `fetch()` can't pass. Point it at FlareSolverr's `/v1` endpoint (e.g. `homeassistant.local:8191/v1`). Nothing runs inside this image for it - without it configured, a Turnstile-blocked download just fails with a clear log message.
